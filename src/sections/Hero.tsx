...
const Hero = () => {
  ...
  const handleCopyEmail = (e: React.MouseEvent) => {
    e.preventDefault();
    navigator.clipboard.writeText("vibekbin305@gmail.com");
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-screen">
        ...
        <div className="mt-8 pointer-events-auto">
          <Magnetic>
            <a 
              ...
              className="group flex items-center gap-3 px-6 py-3 rounded-full bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 hover:border-purple-500/50 transition-all duration-300 cursor-pointer"
            >
      ...
      */}
      <div className={`fixed bottom-6 left-6 md:bottom-12 md:left-12 z-20 flex flex-col items-center gap-4 transition-all duration-1000 delay-[1500ms] ${
      ...
      */}
      <div className={`fixed bottom-10 right-6 md:right-10 z-50 px-6 py-4 rounded-2xl bg-[#0a0a0a] border border-white/10 text-white shadow-[0_10px_40px_rgba(168,85,247,0.1)] backdrop-blur-xl transition-all duration-500 flex items-center gap-4 ${toastVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
        <div className="w-2.5 h-2.5 rounded-full bg-purple-400 animate-pulse shadow-[0_0_10px_rgba(192,132,252,0.8)]"></div>
        <span className="text-sm font-medium tracking-wide">Email copied to clipboard!</span>
      </div>
    </section>
  );
};
